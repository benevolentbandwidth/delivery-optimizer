              ...
              return (
                <li
                  ...
                  className={`rounded-[24px] border overflow-hidden ${
                    isEditMode ? "border-[#6CCBBE] bg-white" : "border-zinc-300 bg-white"
                  }`}
                >
                  <button
                    type="button"
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0 pt-2.5">
                            ...
                            <div className="mt-1 text-[13px] font-medium leading-none text-zinc-900">{route.driverName}</div>
                          </div>
                        </div>
                        <div className="mt-3 grid grid-cols-3 gap-4">
                          <div>
                            <div className="text-[10px] uppercase tracking-wide text-zinc-500">STOPS</div>
                            <div className="mt-1 text-[15px] font-semibold leading-none text-zinc-900">{sortedStops.length}</div>
                          </div>
                          <div>
                            <div className="text-[10px] uppercase tracking-wide text-zinc-500">DISTANCE</div>
                            <div className="mt-1 text-[15px] font-semibold leading-none text-zinc-900 tabular-nums">{route.distanceMi != null ? `${route.distanceMi}mi` : "—"}</div>
                          </div>
                          <div>
                            <div className="text-[10px] uppercase tracking-wide text-zinc-500">TIME</div>
                            <div className="mt-1 text-[15px] font-semibold leading-none text-zinc-900 tabular-nums">{formatEstTime(route.estimatedTimeMinutes)}</div>
                          </div>
                        </div>
                        <p className="mt-3 flex items-center gap-2 text-[12px] text-zinc-700">
                          <svg className="h-4 w-4 text-zinc-400" viewBox="0 0 20 20" fill="none" aria-hidden>
                            <path d="M2.5 5.5A1.5 1.5 0 0 1 4 4h7.5A1.5 1.5 0 0 1 13 5.5V7h1.75c.4 0 .77.16 1.06.44l1.75 1.75c.28.28.44.66.44 1.06v2A1.75 1.75 0 0 1 16.25 14h-.6a2.15 2.15 0 0 1-4.2 0h-3.9a2.15 2.15 0 0 1-4.2 0H2.75A1.75 1.75 0 0 1 1 12.25V7.25A1.75 1.75 0 0 1 2.75 5.5h-.25Z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
                            <circle cx="5.4" cy="14" r="1.15" stroke="currentColor" strokeWidth="1.4" />
                            <circle cx="13.6" cy="14" r="1.15" stroke="currentColor" strokeWidth="1.4" />
                          </svg>
