...
export default function ResultsPage() {
  const [routes, setRoutes] = useState<Route[]>(() => [
    mockRouteToRoute(mockRouteJson as MockRouteJson),
  ]); // Lazy initializer: compute initial routes once so first render already has data (no empty flash, no extra re-render)
    ...
    <main className="h-screen flex flex-col overflow-hidden">
      ...
      <header className="flex items-center gap-2 p-4 shrink-0 border-b border-zinc-200 bg-white">
        ...
        >
          ...
          >
            {" "}
            {/* SVG: hamburger icon inside the button */}
            <path
              strokeLinecap="round"
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          ...
          />{" "}
          {/* Passing the current list of routes and current edit mode state to the sidebar component */}
        </div>
