...
TEST(TrafficForecastOptimizerTest, ReadsTrafficLegsFromVroomSteps) {
  ...
  const std::vector<deliveryoptimizer::api::TrafficLeg> legs =
      deliveryoptimizer::api::ReadTrafficLegs(output,
                                              std::chrono::sys_seconds{std::chrono::seconds{1800}});

  ASSERT_EQ(legs.size(), 2U);
  EXPECT_EQ(legs[0].departure_time.time_since_epoch(), std::chrono::seconds{1800});
  EXPECT_EQ(legs[1].departure_time.time_since_epoch(), std::chrono::seconds{2520});
